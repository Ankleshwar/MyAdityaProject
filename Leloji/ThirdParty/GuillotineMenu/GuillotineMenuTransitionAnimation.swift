...
open class GuillotineTransitionAnimation: NSObject {
  ...
  fileprivate func showHostTitleLabel(_ show: Bool, animated: Bool) {
    ...
    if let supportView = supportView {
      titleLabel.center = CGPoint(
        x: supportView.frame.height / 2,
        y: supportView.frame.width / 2
      )
    }
  ...
  fileprivate func updateChromeView() {
    chromeView = {
      let size = CGRect(
        x: 0,
        y: menu.view.frame.height,
        width: menu.view.frame.width,
        height: menu.view.frame.height
      )
  ...
  fileprivate func animateMenu(_ view: UIView, context:UIViewControllerContextTransitioning) {
    ...
    if mode == .presentation {
      ...
      view.transform = CGAffineTransform.identity.rotated(
        by: degreesToRadians(initialMenuRotationAngle)
      )
      view.frame = CGRect(
        x: 0,
        y: -view.frame.height + topOffset,
    ...
    collisionBehaviour.addBoundary(
      withIdentifier: "collide" as NSCopying,
